...
VOLATILITY_WINDOWS = {"H": 24, "D": 30, "M": 12}
...
def configure_page() -> None:
    """
    Configure la page Streamlit (titre, favicon, largeur, sidebar).

    Cette étape doit être exécutée avant tout rendu afin que
    Streamlit applique correctement la configuration globale.
    """
    st.set_page_config(
...
def inject_custom_style() -> None:
    """
    Injecte la feuille de style CSS définie ci-dessus.

    Cela permet de conserver une palette sombre uniforme et
    d’obtenir des cartes de métriques personnalisées.
    """
...
def get_ma_windows(freq: str) -> Tuple[int, int]:
    """
    Retourne les longueurs des moyennes mobiles courte/longue.

    Parameters
    ----------
    freq : str
        Code de fréquence pandas (H, D, M).
    """
...
def get_volatility_window(freq: str) -> int:
    """
    Retourne la fenêtre de volatilité glissante associée à la fréquence.
...
def render_metric_card(label: str, value: str, delta: float | None = None) -> None:
    """
    Affiche une carte de métrique stylisée avec un delta optionnel.
    ...
    value : str
        Valeur principale déjà formatée (monnaie, pourcentage, etc.).
    delta : float | None
        Variation exprimée en pourcentage (positive ou négative).
    """
    delta_html = ""
...
def load_raw_data(path: Path = DATA_FILE) -> pd.DataFrame:
    """
    Charge les données OHLCV brutes depuis le CSV minute.
...
def get_aggregated_data(freq: str) -> pd.DataFrame:
    """
    Agrège les données au niveau de granularité souhaité et calcule
    les indicateurs (retours, MA, volatilité, range).
...
def get_time_bounds(path: Path = DATA_FILE) -> Tuple[date, date]:
    """
    Retourne les bornes temporelles du dataset.
...
def filter_dataframe(
    ...
    """
    Filtre les données agrégées selon un intervalle de dates.

    Parameters
    ----------
    df_view : pd.DataFrame
        Données agrégées.
    selected_range : Sequence[date] | None
        Tuple (début, fin) provenant du sélecteur Streamlit.
    """
...
def render_metrics(df_filtered: pd.DataFrame) -> None:
    """
    Affiche les indicateurs synthétiques liés au sous-échantillon.

    Parameters
    ----------
    df_filtered : pd.DataFrame
        Sous-ensemble filtré selon la plage temporelle.
    """
...
def render_price_chart(
    ...
    """
    Trace l’évolution du prix avec les moyennes mobiles optionnelles.

    Parameters
    ----------
    df_filtered : pd.DataFrame
    show_ma : bool
        Active/désactive les courbes de MA.
    freq : str
        Fréquence utilisée, nécessaire pour le libellé.
    """
...
def render_candlestick(df_filtered: pd.DataFrame) -> None:
    """
    Affiche un graphique en chandelier sous-échantillonné.

    Parameters
    ----------
    df_filtered : pd.DataFrame
        Données filtrées (post sélection utilisateur).
    """
...
def render_volume(df_filtered: pd.DataFrame) -> None:
    """
    Affiche un histogramme des volumes agrégés.
...
def render_price_volume_relation(df_filtered: pd.DataFrame) -> None:
    """
    Affiche un nuage de points prix vs volume coloré par variation.
...
def render_volatility(df_filtered: pd.DataFrame, freq: str) -> None:
    """
    Affiche la volatilité glissante dont la fenêtre dépend de la fréquence.
...
def render_correlation_heatmap(df_filtered: pd.DataFrame) -> None:
    """
    Affiche une heatmap des corrélations principales (Close/Volume/etc.).
...
def render_tables(df_filtered: pd.DataFrame) -> None:
    """
    Affiche deux tableaux (prix et rendements) pour la période filtrée.
...
def render_footer() -> None:
    """Affiche le pied de page avec rappel du contexte pédagogique."""
    st.markdown("---")
    st.markdown(
        """
...
def build_sidebar(
    ...
    """
    Construit la barre latérale (granularité, plage de dates, options).

    Parameters
    ----------
    min_date, max_date : date
        Bornes globales du dataset.
    """
    ...
    date_range = st.sidebar.date_input(
        ...
        min_value=min_date,
        max_value=max_date,from datetime import date, timedelta
from pathlib import Path
from typing import Sequence, Tuple
...
def main() -> None:
    """
    Point d'entrée : configure la page, charge les données et orchestre
    l’affichage des différentes sections/tabs.
    """
    configure_page()
    inject_custom_style()
    st.title("📊 Analyse Exploratoire du Bitcoin")
